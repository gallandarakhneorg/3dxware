...
bool MSWindowKeyManager::isModifierKey(long wKey)
{
	unsigned long a = 0;
...
ModifierKey MSWindowKeyManager::toModifier(long wKey)
{
	unsigned long a = 0;
	unsigned long b = modifierMapSize - 1;
	unsigned long c = (a+b)/2;
...
EventKey MSWindowKeyManager::toKey(long wKey)
{
	unsigned long a = 0;
...
MSWindowKeyManager::MSWindowKeyManager(): lastKey(KEY_VOID), hasNewKeyValue(false), modifiers(0)
{
	ensureSortedMaps();
}

MSWindowKeyManager::MSWindowKeyManager(const MSWindowKeyManager& manager)
: lastKey(manager.lastKey), hasNewKeyValue(manager.hasNewKeyValue), modifiers(manager.modifiers)
{
	ensureSortedMaps();
}

MSWindowKeyManager::~MSWindowKeyManager()
{
}

MSWindowKeyManager& MSWindowKeyManager::operator=(const MSWindowKeyManager& manager)
{
...
MSWindowKeyManager& MSWindowKeyManager::operator+=(long wKey)
{
	if (isModifierKey(wKey)) {
		modifiers = modifiers | toModifier(wKey);
...
MSWindowKeyManager::operator EventKey() const
{
	return lastKey;
}

MSWindowKeyManager::operator ModifierKeyField() const
{
	return modifiers;
}

bool MSWindowKeyManager::hasNewKey() const
{
	return hasNewKeyValue;
}

bool MSWindowKeyManager::isVoid() const
{
